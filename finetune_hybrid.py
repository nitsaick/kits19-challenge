...
def main(epoch_num, batch_size, lr, num_gpu, data_path, log_path, du2d_path, hybrid_path, eval_intvl, cp_intvl,
         vis_intvl,
         num_workers):
    # prepare
    ...
    if log_path.exists() and len(list(log_path.glob('*'))) > 0:
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        try:
            ...
            cp_file = cp_path / 'hybd_INTERRUPTED.pth'
            cp.save(epoch, net.module, optimizer, str(cp_file))
            cp_file = cp_path / 'du2d_INTERRUPTED.pth'
            cp.save(epoch, dense_unet_2d.module, optimizer, str(cp_file))
            return

        if valid_score > best_score:
            best_score = valid_score
            best_epoch = epoch
            cp_file = cp_path / 'hybd_best.pth'
            cp.save(epoch, net.module, optimizer, str(cp_file))
            cp_file = cp_path / 'du2d_best.pth'
            cp.save(epoch, dense_unet_2d.module, optimizer, str(cp_file))
            print('Update best acc!')

        if (epoch + 1) % cp_intvl == 0:
            cp_file = cp_path / f'hybd_cp_{epoch + 1:03d}.pth'
            cp.save(epoch, net.module, optimizer, str(cp_file))
            cp_file = cp_path / f'du2d_cp_{epoch + 1:03d}.pth'
            cp.save(epoch, dense_unet_2d.module, optimizer, str(cp_file))
...
def training(net, dense_unet_2d, dataset, criterion, optimizer, scheduler, batch_size, num_workers, vis_intvl, logger, epoch):
    ...
    for batch_idx, (imgs, labels, idx) in enumerate(tbar):
        ...
        outputs_2d_list = []
        loss_2d = []
        for stack_img, stack_label in zip(stack_imgs, stack_labels):
