...
from dataset import kits19_roi_volume
from dataset.transform import Compose, PadAndResize
...
class Trainer:
    def __init__(self, net, dataset, criterion, optimizer, scheduler, start_epoch, epoch_num, batch_size,
                 dense_unet_2d):
        self.net = net
        ...
        self.dense_unet_2d = dense_unet_2d
    ...
    def run(self):
        ...
        self.dense_unet_2d = torch.nn.DataParallel(self.dense_unet_2d, device_ids=self.gpu_ids).cuda().eval()
        ...
        for batch_idx, (imgs, labels, idx) in enumerate(tbar):
            ...
            with torch.no_grad():
                for stack_img in stack_imgs:
                    feat_2d, outputs_2d = self.dense_unet_2d(stack_img)
            ...
            outputs = self.net(input_concat, feat_2d)
            ...
            if self.visualize_iter_interval > 0 and batch_idx % self.visualize_iter_interval == 0:
                ...
                imgs, labels, outputs = self.dataset.vis_transform(imgs[:, :, :, :, slice_num // 2],
                                                                   labels[:, :, :, slice_num // 2],
                                                                   outputs[:, :, :, slice_num // 2])
...
if __name__ == '__main__':
    ...
    dataset = kits19_roi_volume(data_path, slice_num=12, valid_rate=0.3,
                                train_transform=train_transform,
                                valid_transform=valid_transform,
                                specified_classes=[0, 1, 2])
    
    net = HybridNet(in_ch=4)
    ...
    cp_file = Path('runs/DenseUNet2D/best.pth')
    dense_unet_2d = DenseUNet2D()
    dense_unet_2d, _, _ = cp.load_params(dense_unet_2d, root=str(cp_file))
    ...
    trainer = Trainer(net, dataset, criterion, optimizer, scheduler, epoch_num=EPOCH, start_epoch=start_epoch,
                      batch_size=BATCH_SIZE, dense_unet_2d=dense_unet_2d)
    trainer.run()
