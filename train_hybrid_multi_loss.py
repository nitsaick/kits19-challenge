...
from loss import GeneralizedDiceLoss, BoundaryLoss, DiceLoss
from loss.util import class2one_hot, one_hot2dist
...
def main(epoch_num, batch_size, lr, num_gpu, data_path, log_path, du2d_path, resume, eval_intvl, cp_intvl, vis_intvl,
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        try:
            loss = training(net, dense_unet_2d, dataset, celoss, gdloss, bdloss, optimizer, scheduler, batch_size, num_workers,
...
def training(net, dense_unet_2d, dataset, celoss, gdloss, bdloss, optimizer, scheduler, batch_size, num_workers, vis_intvl, logger, epoch):
    ...
    for batch_idx, (imgs, labels, idx) in enumerate(tbar):
        ...
        loss1_3d = celoss(cls_3d, labels)
        loss1_hff = celoss(outputs, labels)

        cls_3d = F.softmax(cls_3d, dim=1)
        outputs = F.softmax(outputs, dim=1)

        labels_onehot = class2one_hot(labels, 3)
        labels_distmap = torch.Tensor(one_hot2dist(labels_onehot.cpu().numpy())).cuda()

        loss2_3d = gdloss(cls_3d, labels_onehot)
        loss2_hff = gdloss(outputs, labels_onehot)

        loss3_3d = bdloss(cls_3d, labels_distmap)
        loss3_hff = bdloss(outputs, labels_distmap)

        loss = loss1_3d + loss1_hff + loss2_3d + loss2_hff + loss3_3d + loss3_hff
